...
coord_flagger #(.NARROW_WIDTH(NARROW_WIDTH), .NARROW_FRACTIONAL_BITS(NARROW_FRACTIONAL_BITS), .INTEGER_BITS(INTEGER_BITS)) left_overflow_flag 
(
.coordinate(left_multiply_result),
.is_wide(is_wide),
.flag(left_o_flag)
);
coord_flagger #(.NARROW_WIDTH(NARROW_WIDTH), .NARROW_FRACTIONAL_BITS(NARROW_FRACTIONAL_BITS), .INTEGER_BITS(INTEGER_BITS)) right_overflow_flag
(
.coordinate(right_multiply_result),
.is_wide(is_wide),
.flag(right_o_flag)
);
...
typedef enum {C_IDLE, PRE_ALTER_SUM, ALTER_SUM, X_SQUARED, Y_SQUARED_1, Y_SQUARED_2, TWO_I_XY, ADD_COORD_X,ADD_COORD_Y, ADD_JULIA_X,ADD_JULIA_Y, DONE} thread_cycle;
...
// reg signed [NARROW_WIDTH-1:0] magnitude_reg_1;
// reg signed [NARROW_WIDTH-1:0] magnitude_reg_2;
...
logic signed [2*NARROW_WIDTH-1:0] right_mac_add;
logic left_mac_sub;
logic right_mac_sub;
        ...
        SPLIT : begin
            ...
            case(left_cycle) 
                ...
                Y_SQUARED_1 : begin 
                    ...
                    left_mac_add = left_multiply_result;              
                end
                Y_SQUARED_2 : begin
                    // dont know how to do this
                    left_mac_sub = 1'b1;
                    left_multiply_mode = 2'b01;
                    left_mac_add = sum_x_reg_1;
                end
                TWO_I_XY : begin
                    left_multiply_mode = 2'b10;
                end
                ADD_COORD_X : begin
                    multiply_operand_A_1 = {2'b01,{NARROW_FRACTIONAL_BITS{1'b0}}};
                    multiply_operand_B_1 = starting_x_reg_1;
                    left_multiply_mode = 2'b10;
                    left_mac_add = sum_x_reg_1;
                end
                ...
                ADD_COORD_Y : begin
                    multiply_operand_A_1 = {2'b01,{NARROW_FRACTIONAL_BITS{1'b0}}};
                    multiply_operand_B_1 = starting_y_reg_1;
                    left_multiply_mode = 2'b10;
                    left_mac_add = {sum_y_reg_1[2*NARROW_WIDTH-2:0],1'b0};
                end

                ADD_JULIA_X : begin
                    multiply_operand_A_1 = {2'b01,{NARROW_FRACTIONAL_BITS{1'b0}}};
                    multiply_operand_B_1 = julia_c_x;
                    left_multiply_mode = 2'b10;
                    left_mac_add = sum_x_reg_1;
                end
                ...
                ADD_JULIA_Y : begin
                    multiply_operand_A_1 = {2'b01,{NARROW_FRACTIONAL_BITS{1'b0}}};
                    multiply_operand_B_1 = julia_c_y;
                    left_multiply_mode = 2'b10;
                    left_mac_add = {sum_y_reg_1[2*NARROW_WIDTH-2:0],1'b0};      
                end          
                default : begin
            ...
            case(right_cycle) 
                ...
                Y_SQUARED_1 : begin 
                    ...
                    right_mac_add = right_multiply_result;              
                end
                Y_SQUARED_2 : begin
                    // dont know how to do this
                    right_mac_sub = 1'b1;
...
(* keep_hierarchy = "yes" *) mac #(.NARROW_WIDTH(NARROW_WIDTH)) left_multiply 
(
    .clk(clk),
...
(* keep_hierarchy = "yes" *) mac #(.NARROW_WIDTH(NARROW_WIDTH)) right_multiply 
(
    .clk(clk),
...
(
    .magnitude(left_multiply_result),
...
(
    .magnitude(right_multiply_result),
        ...
        if(start_left) begin
            ...
            case(left_cycle)
                PRE_ALTER_SUM : begin
                    ...
                    if(left_o_flag) left_cycle <= DONE;
                    else left_cycle <= ALTER_SUM;
                end

                ALTER_SUM : begin

                    // take sum_alter_y input from multiply result
                ...
                X_SQUARED : begin
                // $display("sum_x_reg_1 = %36d (raw int)", (sum_x_reg_1));
                // $display("sum_y_reg_1 = %36d (raw int)", ((sum_y_reg_1)));

                    left_cycle <= Y_SQUARED_1;
                end
                ...
                Y_SQUARED_1 : begin
                    ...
                    // $display("x squared = %36d (raw int)", (left_multiply_result));
                
                

                    left_cycle <= Y_SQUARED_2;
                ...
                //save multiply result to x sum register

                    sum_x_reg_1 <= left_multiply_result;
                
                    if(julia_type) left_cycle <= ADD_JULIA_X;
                    else left_cycle <= ADD_COORD_X;
                end

                ADD_JULIA_X : begin

                    //load result to y reg

                    
                    sum_y_reg_1 <= left_multiply_result;
                    iteration_reg_1 <= iteration_reg_1 + 1;  
                    left_cycle <= ADD_JULIA_Y;                  
                end
                ...
                ADD_COORD_X : begin
                    ...
                    sum_y_reg_1 <= left_multiply_result;
                    iteration_reg_1 <= iteration_reg_1 + 1; 
                    left_cycle <= ADD_COORD_Y; 
                    // $display("x^2-y^2 = %36d (raw int)", $signed(sum_x_reg_1)); 
                    // $display("multiply_op_1 = %18b (raw int)", ((multiply_operand_A_1)));
                    // $display("multiply_op_2 = %18b (raw int)", ((multiply_operand_B_1)));
                    // $display("mac code = %2d " ,(left_multiply_mode));
    
                    // $display("mac add input = %36b (raw int)", ((left_mac_add)));
     

                end

                ADD_JULIA_Y : begin
                    //load result to x reg
                    sum_x_reg_1 <= left_multiply_result;
                    if(left_o_flag) left_cycle <= DONE;
                    else left_cycle <= PRE_ALTER_SUM;
                end
                ...
                ADD_COORD_Y : begin
                    ...
                    // $display("x^2 - y^2 + c_x = %36b (raw int)", ((left_multiply_result)));
                    if(left_o_flag) left_cycle <= DONE;
                    else left_cycle <= PRE_ALTER_SUM;
        ...
        if(start_right) begin
            ...
            case(right_cycle)
                PRE_ALTER_SUM : begin
                    ...
                    if(right_o_flag) right_cycle <= DONE;
                ...
                //save multiply result to x sum register
                    ...
                    if(julia_type) right_cycle <= ADD_JULIA_X;
                    else right_cycle <= ADD_COORD_X;
                end
                ...
                ADD_JULIA_Y : begin
                    ...
                    sum_x_reg_2 <= right_multiply_result;
                    if(right_o_flag) right_cycle <= DONE;
                    else right_cycle <= PRE_ALTER_SUM;
                end
