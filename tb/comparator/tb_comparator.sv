...
module tb_comparator;
    ...
    logic        rst;
    logic        sched_reset;
    logic [7:0]  top_left_x, top_left_y;             // 8-bit coords
    logic [8:0]  quad_size_x, quad_size_y;           // COORD_W:0 (split x/y)
    logic [10:0] expected_count;
    logic        comp_valid;
    logic [21:0] comp_data;   // { colour[5:0], y[7:0], x[7:0] }
    logic        comp_pop;
    ...
    comparator u_comp (
        ...
        .quad_size_x   (quad_size_x),
        .quad_size_y   (quad_size_y),
    ...
    // comp_data = { colour[5:0], y[7:0], x[7:0] }
    task automatic feed(input logic [7:0] x, y, input logic [5:0] col);
        comp_data  = { col, y, x };
        comp_valid = 1;
    ...
    // Configure comparator for a new quad and pulse sched_reset (square quads:
    // the single qsize drives both x and y).
    ...
    initial begin
        ...
        top_left_x=0; top_left_y=0; quad_size_x=0; quad_size_y=0; expected_count=0;
        ...
        // Current comparator suppresses `complete` whenever `differ` is set (the
        // !differ && !this_differs guard), so a box with any mismatch never reports
        // complete — preventing the scheduler from flood-filling a non-uniform box.
        check(!complete, "complete suppressed when last entry differs");
        ...
        suite("COMPLETE — differ suppresses complete (cannot both be high)");
        hard_reset();
        load_quad(8'd0, 8'd0, 8'd16, 11'd3);
        feed(8'd0, 8'd0, 4'hA);
        feed(8'd1, 8'd0, 4'hB);   // differ
        feed(8'd2, 8'd0, 4'hA);   // 3rd entry — complete would fire but differ blocks it
        tick(1);
        $display("    differ=%b complete=%b (complete must stay low once differ set)", differ, complete);
        check(differ,    "differ asserted");
        check(!complete, "complete suppressed because differ is set");

        suite("COMPLETE — expected_count=1 needs ref latched (no complete on very first entry)");
        hard_reset();
        load_quad(8'd0, 8'd0, 8'd16, 11'd1);
        feed(8'd0, 8'd0, 4'h9);   // first entry only sets ref_valid (registered) this cycle
        tick(1);
        $display("    Single-entry: complete=%b differ=%b", complete, differ);
        // complete checks the REGISTERED ref_valid, which is still 0 on the very
        // first entry, so an expected_count=1 box completes on the SECOND in-bounds
        // entry, not the first. No differ either way.
        check(!complete, "no complete on the very first entry (ref not yet latched)");
        ...
        comp_valid = 1; comp_data = {6'h01, 8'd0, 8'd0};
        ...
        comp_valid = 1; comp_data = {6'h0F, 8'd0, 8'd0};  // OOB x=0,y=0
        #1;
